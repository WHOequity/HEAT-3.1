...
cov <- function(dat, bs=FALSE, bsiter = 200, simulated_results = NULL){
  ...
  if(SEuseful){
    ...
    # This formula is not going to be corrected if the cov estimate is based on
  ...
  return(list(inequal.cov=inequal.cov, se.cov.boot=se.boot,  se.cov.formula=se.formula, 
              se.lowerci.cov = ci$l, se.upperci.cov = ci$u))  
}
